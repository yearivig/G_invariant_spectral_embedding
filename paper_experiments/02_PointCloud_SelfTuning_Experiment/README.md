...
Based on the latest comparison runs in this repository, **self-tuning methods are the best performers** for this point-cloud dataset.

- Best overall in our tests: `invariant_features_self_tuning`
- Baseline non-invariant method: `none`
- Self-tuned counterpart for direct comparison: `none_self_tuning`

For paper figures, compare `none` vs `none_self_tuning` using the same `num-points` and `noise` settings.
...
\]where \(d_k(x_i)\) is the distance to the \(k\)-th nearest neighbor (excluding self).Use `--knn-k 15` to control \(k\) (default is 15).
