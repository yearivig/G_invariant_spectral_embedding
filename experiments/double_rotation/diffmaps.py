...
Embedding variants:
  ...
  - mydiffmap_so2_integral() : SO(2)-invariant diffusion maps (Haar-averaged kernel)
...
# ---------------------------------------------------------------------------
# SO(2)-invariant integral (Haar-averaged) affinity matrix
# ---------------------------------------------------------------------------


def compute_so2_integral_affinity(
    images: np.ndarray | torch.Tensor,
    num_rotations: int = 300,
    sigma2: float | torch.Tensor = 1.0,
    device: DeviceStr | None = None,
    chunk_size: int = 8,
) -> torch.Tensor:
    """
    Compute the pairwise SO(2)-invariant integral (Haar-averaged) affinity matrix.

    For each pair (i, j), computes:
        K(i, j) = mean_{k} exp( -||image_i - R_k(image_j)||^2 / sigma2 )

    This averages the Gaussian kernel over the group, rather than taking
    the min distance.
    ...
    images : (N, H, W) array or tensor
    num_rotations : number of SO(2) samples
    sigma2 : bandwidth (sigma squared) for the Gaussian kernel
    device : PyTorch device string
    chunk_size : process this many images at a time to save GPU memory

    Returns
    -------
    affinity : (N, N) tensor of Haar-averaged affinities
    """
    dev = _get_device(device)
    ...
    n = data.shape[0]
    affinity = torch.zeros((n, n), dtype=torch.float32, device=dev)
    angles = torch.linspace(0, 2 * np.pi, num_rotations, device=dev)
    ...
    for i in range(0, n, chunk_size):
        ...
        for j in range(i, n, chunk_size):
            ...
            # Accumulate sum of Gaussian kernel values over rotations
            kernel_sum = torch.zeros((i_end - i, j_end - j), device=dev)
            ...
            for k in range(0, num_rotations, rot_batch_size):
                ...
                for idx_i, img_i in enumerate(chunk_i):
                    diffs = img_i.unsqueeze(0) - rotated_j  # [num_angles, chunk_j_size, H, W]
                    d2 = (diffs.reshape(k_end - k, j_end - j, -1) ** 2).sum(dim=2)
                    kernel_sum[idx_i] += torch.exp(-d2 / sigma2).sum(dim=0)

            # Average over all rotations
            affinity[i:i_end, j:j_end] = kernel_sum / num_rotations
            if i != j:
                affinity[j:j_end, i:i_end] = (kernel_sum / num_rotations).T
...
# ---------------------------------------------------------------------------
# SO(2)-invariant diffusion maps — integral (Haar) kernel
# ---------------------------------------------------------------------------


def mydiffmap_so2_integral(
    images: np.ndarray,
    num_neighbors: int = 20,
    t: int = 10,
    num_rotations: int = 300,
    ...
    """
    SO(2)-invariant diffusion maps using the integral (Haar-averaged) kernel.

    Instead of computing min-distance and then applying a Gaussian,
    this directly averages the Gaussian kernel over the group:
        A(i, j) = mean_{k} exp( -||img_i - R_k(img_j)||^2 / sigma2 )

    The bandwidth sigma2 is estimated from the min-distance matrix (k-NN).
    ...
    dev = _get_device(device)
    n = images.shape[0]
    logger.info(f"mydiffmap_so2_integral: N={n}, t={t}, rotations={num_rotations}, device={dev}")

    # First compute the min-distance matrix to estimate bandwidth
    dist_mat = compute_so2_distance_matrix(images, num_rotations=num_rotations, device=str(dev))
    sorted_d, _ = torch.sort(dist_mat, dim=1)
    sigma2 = sorted_d[:, 1 : num_neighbors + 1].mean() ** 2

    # Now compute the integral affinity matrix using that bandwidth
    A = compute_so2_integral_affinity(
        images, num_rotations=num_rotations, sigma2=sigma2.item(),
        device=str(dev),
    )
